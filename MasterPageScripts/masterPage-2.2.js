...
$(document).ready(function () {
    ...
    $("#footerCopyright").html("© TFW Labs, Inc.");
...
function GetNavigationsDetails(currentLanguage) {
    ...
    //if (localStorageLanguage == null || localStorageLanguage == "" || localStorageLanguage == "null") {
    var Ownurl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('Navigation')/items?select=ParentId,ExternalLink,Status,Postion,Menu,URL,Language2nd,Language3rd,Language4th,Language5th,Language6th,Language7th,Language8th,Language9th,Language10th,DepartmentID,ID&$orderby=Postion asc&$filter=CompanyId  eq '" + companyIdNavigation + "' and Status eq 1";
    $.ajax({
        url: Ownurl,
        headers: {
            Accept: "application/json;odata=verbose"
...
function CommonNavigation(items, currentLanguage) {
    ...
    var MyProfile = _spPageContextInfo.webServerRelativeUrl + "/Pages/EmployeeDetails.aspx?WebAppId=" + CompanyId + "&mode=editview&MyProfile=true&department=&employeedIddetails=" + newItemID + "&sourcelocation=" + _spPageContextInfo.webAbsoluteUrl + "";
    ...
    for(var index = 0; index < items.length; index++) {
        ...
        navigationItem.ExternalLink=items[index].ExternalLink;
        ...
        if(items[index].ParentId == 0) {
            ...
            if(items[index].Title == "Always") {
                if(items[index].ExternalLink==true)
                {
                    mainContent += '<li class="navItem">';
                    mainContent += "<a href='" + items[index].URL + "' target='_blank' title='" + ParentMenu + "'>" + ParentMenu + "  </a>";
                    mainContent += "<span class=\"spanMenu\" style=\"display:none\" >" + ItemID + "</span>";
                    mainContent += "</li>";
                }
                else
                ...
                if(IsBpassModules || LicIntranet == "All") {
                    mainContent += '<li class="navItem">';
                    mainContent += "<a href='" + ParentUrl + "' title='" + ParentMenu + "'>" + ParentMenu + "  </a>";
                    mainContent += "<span class=\"spanMenu\" style=\"display:none\" >" + ItemID + "</span>";
                    mainContent += "</li>";
                } else if(LicIntranet > 0 || IsBpassModules) {
                    if(EmpLinceseEncript.search(/Internet/i) > 0 && EmpLinceseEncript.split("|")[0].toLowerCase() == _spPageContextInfo.userLoginName.toLowerCase()) //now go to empolyee list to check license of loged user
                    {
                        mainContent += '<li class="navItem">';
                        mainContent += "<a href='" + ParentUrl + "' title='" + ParentMenu + "'>" + ParentMenu + "  </a>";
...
function BuildChildMenusHTML(parentThisControl, PresentID, currentLanguage) {
    ...
    if (childMenusCollection.length > 0) {
        ...
        for (var index = 0; index < childMenusCollection.length; index++) {
            ...
            {
                if(childMenusCollection[index].ExternalLink==true)
                {
                    childHTML += "<li style=\"padding: 8px 0 0 0;\"><a title=" + ChildMenu + " target='_blank'  href=" + childMenusCollection[index].URL + " onclick='SetDepartmentDetailsInCookies(" + companyIdNavigation + "," + DepartmentID + ")' >" + ChildMenu + "</a></li>";
...
function GetLogoUrl(currentLanguage) {
    ...
    if (localStorageLanguage == null || localStorageLanguage == "" || localStorageLanguage == "null") {
                ...
                if (items.length > 0) {
                    for (var index = 0; index < items.length; index++) {
                        ...
                        if (data.d.results[index][companyLogo] != null) {
                            ...
                            localStorage.setItem("SiteUrl",g_BusinessDomain)
        ...
        $("#navbarLogoLink").attr("href",localStorage.getItem('SiteUrl'));
        var storageDateTimeStamp = new Date(localStorageData.TimeStamp);
        storageDateTimeStamp.setHours(0, 0, 0, 0);
        var currentDate = new Date();
        currentDate.setHours(0, 0, 0, 0);
...
function SetStoredTheme() {
    ...
    //if (Logged_InPageName == "Myworkplace.aspx" || Logged_InPageName == "Document.aspx") {
        if (SetMode == "DarkMode") {
            ...
            $("#dark-light-side-opt .dark-theme-btn-label").text("Dark Mode");
            $("#dark-light-side-opt span.material-icons").html("dark_mode");
            if (Logged_InPageName == "DefaultPage.aspx") $(document.body).attr('style', 'background-color: hsla(206, 100%, 95%, 1) !important');
            else $(document.body).attr('style', 'background-color: hsla(0, 0%, 97%, 1) !important');
        }
    //}
    /*else {
        $(document.body).removeClass('dark-theme');
